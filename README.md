# Helm Chart - Grupo 118 Fase Final
...
git clone https://github.com/yourusername/final-challenge-grupo-118-helm-chart.git
cd grupo118fasefinal

# Install the chart with default values
helm install my-api ./helm/grupo118fasefinal

# Install with custom namespace
helm install my-api ./helm/grupo118fasefinal --namespace my-namespace --create-namespace
```
...
# Install from ACR
helm install my-api oci://your-acr-name.azurecr.io/helm/grupo118fasefinal --version 1.0.0
```
...
```bash
helm install my-api ./helm/grupo118fasefinal -f custom-values.yaml
```
...
service:
  instances:
    - name: grupo118fasefinal-service
      type: ClusterIP
      port: 80
      targetPort: 8080
...
| `size` | Storage size request | `20Gi` |
| `accessModes` | Volume access modes | `[ReadWriteOnce]` |
| `storageClassName` | Storage class for PVC | `managed-csi` |
...
- `managed-csi` is the recommended storage class for Azure Disk
...
```bash
helm install payment-api ./helm/grupo118fasefinal -f payment-api-values.yaml
```
...
```bash
helm install order-api ./helm/grupo118fasefinal -f order-api-values.yaml
```
...
helm upgrade my-api ./helm/grupo118fasefinal -f custom-values.yaml

# Upgrade with specific values
helm upgrade my-api ./helm/grupo118fasefinal --set image.tag=v2.0.0
```
...
# Pull the chart
helm pull oci://your-acr-name.azurecr.io/helm/grupo118fasefinal --version 1.0.X

# Install from ACR
helm install my-api oci://your-acr-name.azurecr.io/helm/grupo118fasefinal --version 1.0.X
```
...
Ensure [`acrCredentials`](helm/grupo118fasefinal/values.yaml) are correctly configured.
...
3. Check ingress class matches: [`internalIngress.className`](helm/grupo118fasefinal/values.yaml)

### Debug Mode

```bash
# Dry run to see generated manifests
helm install my-api ./helm/grupo118fasefinal --dry-run --debug

# Template without installing
helm template my-api ./helm/grupo118fasefinal
```

## 📖 Additional Resources

- [Helm Documentation](helm/grupo118fasefinal/README.md) - Detailed chart documentation
- [Chart.yaml](helm/grupo118fasefinal/Chart.yaml) - Chart metadata
- [values.yaml](helm/grupo118fasefinal/values.yaml) - Default values and configuration
...
This Helm chart is provided as-is for Grupo 118 Tech Challenge Fase Final.
...
**Grupo 118 - Tech Challenge Fase Final**